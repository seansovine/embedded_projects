...
that is configured to communicate with a UART over a USB-to-UART bridge.

The bridge device we use is [this one](https://www.amazon.com/dp/B07WX2DSVB), which has the
...
_Testing MCU version in my home workspace:_
...
This is a simple UART echo server implemented on the STM32F407-DISC1 development board by
ST Microelectronics. For ST development I am now using the CubeMX generated CMake project ->
VS Code STM32 extension pack workflow. I am very happy with this environment. In particular
I like that CubeMX generates a portable CMake project with only the necessary initialization
code, HAL and libary files includes, and it provides for code sections -- those marked with
...
-- that it won't touch if you update the CubeMX configuration and regenerate. The VS Code
extension pack has solid integration with CMake and Clangd.

This microcontroller version of the server can be used to validate that things are setup
correctly on the Linux PC side of things.

### DE10 Lite FPGA design: [uart_echo_de10_lite](./uart_echo_de10_lite/)

This implements the echo server on the Terasic DE10 Lite dev board, which has an Altera Max 10
chip on it. It includes the Verilog source files and the necessary project files to open and
build the design in Quartus.

I got the UART receiver modules from [Nand Land](https://nandland.com/uart-serial-port-module/)
and modified them very slightly to add an asynchronous reset and to allow debugging the internal
state while I was getting the design setup.
...
run it on the device. This can serve as a quick template for Quartus projects for the
DE10 Lite board.
