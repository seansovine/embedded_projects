module led_blinker #(
    ...
    input wire clk,
    input wire rst_n,
    output reg [9:0] LED  // For connection to LEDR9 to LEDR0.
);

    // Register to count clock cycles.
    reg  [31:0] clk_counter;

    // Register to hold curren LED number.
    reg  [ 3:0] current_led;
...
module led_blinker_single #(
    ...
    input wire clk,
    input wire rst_n,
    output reg LED  // LED or other signal to toggle.
);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Toggle LED every 1 / BLINK_RATE seconds.
            if (clk_counter >= (CLK_FREQ / BLINK_RATE - 1)) begin
                clk_counter <= 32'd0;
                LED         <= ~LED;
            end else begin
